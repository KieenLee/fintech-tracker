...
const Goals = () => {
  ...
  const handleSave = () => {
    if (formData.title && formData.targetAmount && formData.deadline) {
      ...
      setEditingGoal(null);
      setFormData({ title: "", description: "", targetAmount: "", currentAmount: "", deadline: "", priority: "Medium" });
    }
  };
    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {goals.map((goal) => {
          ...
          return (
            <Card key={goal.id} className={`transition-all hover:shadow-md animate-scale-in ${isCompleted ? 'border-success' : ''}`}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <CardTitle className="flex items-center gap-2">
                      ...
                      {goal.title}
                      {isCompleted && <span className="text-success text-sm">✓ Completed</span>}
                    </CardTitle>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground">
                    {percentage.toFixed(1)}% complete • ${(goal.targetAmount - goal.currentAmount).toLocaleString()} remaining
                  </div>
                </div>
